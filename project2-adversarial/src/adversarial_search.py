...
class _MinMaxAlgo:
    def _min(self, mdp: MDP[A, S], state: S, max_depth: int) -> float:
        ...
        while len(next_states) > 0:
            ...
            if current_state.current_agent == 0:
                best_value = min(best_value, self._max(mdp, current_state, max_depth-1))
            else:
                next_states += [
    ...
    def _max(self, mdp: MDP[A, S], state: S, max_depth: int) -> float:
        ...
        for action in mdp.available_actions(state):
            ...
            best_value = max(best_value, self._min(mdp, next_state, max_depth-1))

        return best_value

    def minimax(self, mdp: MDP[A, S], state: S, max_depth: int) -> A:
        ...
        for action in mdp.available_actions(state):
            next_state = mdp.transition(state, action)
            value = self._min(mdp, next_state, max_depth-1)
